...
- 統計分析與文字洞察的單一問卷二級頁移除重複 workflow 說明，改用同一套 header KPI 膠囊列。
- 問卷編輯頁（Survey Builder）頂部同步改為 KPI 膠囊列，顯示問卷、題目、回覆、最近回覆，右側保留統計分析、文字洞察、返回問卷列表操作。
- 統計分析頁的資料地圖、描述統計、推論分析改為 tab 工作流；文字洞察頁改為關鍵字摘要、情緒分布、分類規則三個 tab，無箭頭。

### 文字洞察頁重構

- 關鍵字摘要由大型重複說明卡片改成三欄精簡卡片，保留分類、關鍵字、次數、建立改善。
- 關鍵字標籤改為小圓角矩形，使用與文字雲相同的分類色系，並加 1px 邊框。
- 文字雲說明文字集中到文字雲下方，避免每張關鍵字卡重複說明。
- 分類情緒分布改為卡片式 stacked bar，將正向 / 中性 / 負向的文字與數字整合進長條圖內。
- 情緒分類卡片依最大情緒比例顯示背景：正向淡綠、中性淡黃、負向淡紅，並使用 3px 對應色輪廓。
- 分類徽章固定 8 個中文字寬；總筆數移到長條圖右側並以徽章顯示。
- 移除情緒分布 tab 中低資訊密度的文字題清單。

### 關鍵字分類規則管理

- 新增 `KeywordCategoryUpdateView` 與 `/dashboard/keyword-categories/<pk>/update/` 路由。
- 分類規則 tab 每筆規則新增 inline 編輯表單，可修改關鍵字、分類名稱與門檻。
- 新增 / 編輯 / 刪除分類規則後會回到文字洞察的 `#text-rules` tab。
- 編輯時會檢查空白欄位、門檻正整數，以及同一問卷內 keyword 不可重複。

**相關修改：** `feedback/views.py`、`feedback/urls.py`、`templates/feedback/stats_overview.html`、`templates/feedback/text_analysis.html`、`templates/feedback/survey_builder.html`、`static/css/app.css`
